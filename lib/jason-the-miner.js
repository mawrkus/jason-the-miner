...
class JasonTheMiner {
  ...
  _paginate(loader, parser, paginator, loadConfig) {
    return loader.run(loadConfig)
      ...
      .then(results => {
        debug('Paginating..?');
        const nextLoadConfigs = paginator.run({
        ...
        const nextP = nextLoadConfigs.map(nextConfig => {
          return this._paginate(loader, parser, paginator, nextConfig)
            // reflect to avoid Promise.all to fail
            .catch(error => [{ error: this._formatRunError(error) }]);
        });

        return Promise.all(nextP)
          .then(nextResults => nextResults.reduce((all, current) => all.concat(current), results));
      });
  }
