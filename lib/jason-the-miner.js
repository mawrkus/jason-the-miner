...
class JasonTheMiner {
   ...
   */
  constructor({ processors, fallbacks, helpers } = {
    fallbacks: {
      input: 'identity',
      parse: 'html',
      output: 'identity',
  ...
  registerHelper({ category, name, helper }) {
    if (typeof helper !== 'function') {
      throw new TypeError(`Cannot register "${category}" helper "${name}"!`);
    }
  ...
  loadConfig(configFile) {
    ...
    return new Promise((resolve, reject) => {
      fs.readFile(configFile, 'utf8', (error, config) => {
        if (error) {
          reject(error);
          return;
        }
   ...
   * If a pagination config has been specified, 1) and 2) are repeated accordingly.
   * @param  {Object} [options={}]
   ...
   * category. See the "index.js" files in the "processors" subfolders for more info.
