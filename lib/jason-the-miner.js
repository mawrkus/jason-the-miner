...
class JasonTheMiner {
      ...
      .map((concurrency) => {
        ...
        return Bluebird.map(
          ...
          async ({ link, schemaPath, parsedPath }) => {
            debug('Getting schema from path', schemaPath);

            const followLoadParams = this._buildFollowLoadParams(loader, link);
            const followSchema = get(schema, schemaPath);
            ...
            debug('Merging follow result to path', parsedPath);
            debug('followResult', followResult);

            if (parsedPath.length) {
              // TODO: improve me
              const partialResult = get(result, parsedPath);
              set(result, parsedPath, { ...partialResult, ...followResult });
            } else {
  ...
  _buildFollowLoadParams(loader, link) {
