...
class JasonTheMiner {
  ...
  constructor({ fallbacks } = {
    fallbacks: { load: 'identity', parse: 'identity', paginate: 'noop', transform: 'identity' }
  }) {
    ...
    ['load', 'parse', 'paginate', 'transform'].forEach(category => {
      if (fallbacks[category]) {
        this._fallbacks[category] = fallbacks[category];
      }
    });
  ...
  _loadAndParse(loader, parser, paginator, loadParams) {
    return loader.run(loadParams)
      ...
      .then(results => {
        ...
        const { nextParamsArray, throttler } = pagination;

        // recursive case
        const nextP = nextParamsArray.map(nextParams => throttler(() => {
          return this._loadAndParse(loader, parser, paginator, nextParams)
            // reflect to avoid Promise.all to fail
            .catch(error => [{ error: this._formatRunError(error) }]);
        }));
