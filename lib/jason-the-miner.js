...
class JasonTheMiner {
  ...
  async _loadAndParse({ loader, parser, paginator, loadParams, parseSchema }) {
    ...
    if (!follow || !follow.length) {
      debug('Nothing to follow, done harvesting.');
      return result;
    }

    const { concurrency = 1 } = loader.getRunContext().config;

    debug('Following %d link(s) at concurrency=%d...', follow.length, concurrency);
