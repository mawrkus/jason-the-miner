  ...
  _parseSelectorDef(selectorDef, tab) {
    ...
    const [matcher, extractor, filter] = helpersDefs.map(({ category, helperDef }) => {
      ...
      if (!categoryHelpers[name]) {
        debug('%sUnknown "%s" helper "%s"! Using "default".', `${tab}  `, category, name);
        return categoryHelpers.default.bind(categoryHelpers, ...params);
      }

      debug('%sParsed "%s" helper="%s(%s)"', `${tab}  `, category, name, params);
