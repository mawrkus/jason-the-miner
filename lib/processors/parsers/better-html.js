...
class HtmlParser {
      ...
      .forEach((prop) => {
        ...
        switch (typeOfDefinition(definition)) {
          ...
          case 'array': // eslint-disable-line no-case-declarations
            ...
            result[prop] = this._parseListDefinition(definition[0], $context, path.concat([prop, 0]), `${tab}  `);
            break;
  ...
  _parseListDefinition(schema, $context, path, tab) {
    ...
    $elements.each((index, domElement) => {
      ...
      const result = this._parseSchema(schema, $element, path, tab);
      list.push(result);
    });
